...
def action2step(step_data: Dict[str, Any]) -> str:
    ...
    if action_type == 4:
        if step_data["action_type_text"] == 'click':
            ...
            return "{{\"action_type\": {}, \"click_point\": {}}}".format(4, click_point)

        if step_data["action_type_text"] == 'scroll down':
            return "{{\"action_type\": 0}}"
        if step_data["action_type_text"] == 'scroll up':
            return "{{\"action_type\": 1}}"
        if step_data["action_type_text"] == 'scroll left':
            return "{{\"action_type\": 8}}"
        if step_data["action_type_text"] == 'scroll right':
            return "{{\"action_type\": 9}}"

    if action_type == 3:
        typed_text = step_data["type_text"]
        return "{{\"action_type\": {}, \"typed_text\": \"{}\"}}".format(action_type, typed_text)

    return "{{\"action_type\": {}}}".format(action_type)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=[
            '/mnt/vdb1/hongxin_li/uipro_ckpt/0123_UIPro_Qwen2-VL-7B_gnd2planning4336k+Mind2Web-GUIAct_wActRef_s1000_108k/lora/checkpoint-3368',
            'HongxinLi/UIPro-7B_Stage2_Web',
    ...
    parser.add_argument(
        '--root',
        type=str,
        default="/mnt/nvme0n1p1/hongxin_li/UI_training_data/Mind2Web",
        help="Root directory of Mind2Web (overrides DEFAULT_ROOTS auto-detect).",
    )
    parser.add_argument('--device_tag', type=str, default='Web', help="Device tag.")
    parser.add_argument('--max_prev_acts', type=int, default=66, help="Max previous actions in history.")
...
def build_model_postfix(model_path: str) -> str:
    """Create a readable postfix for logging and output dir names."""
    model_path_clean = model_path.replace("lora/", "").replace("merged/", "")
    if "snapshots" in model_path_clean:
        return model_path_clean[
            model_path_clean.find("models--") + 8: model_path_clean.find("snapshots") - 1
        ]
    if len(model_path_clean.split('/')) == 2:
        return model_path_clean.replace('/', '--')
    if 'checkpoint-' in model_path_clean:
        return '/'.join(model_path_clean.replace("lora/merged/", "").split('/')[-2:])
    return model_path_clean.replace('/', '-')


def initialize_model(model_path: str) -> Tuple[Any, Any, str, Dict[str, Any]]:
    """Initialize the model and return (model, tokenizer/processor, postfix, gen_kwargs)."""
    model_path = model_path.rstrip('/ ')
    postfix = build_model_postfix(model_path)
...
def resolve_data_root(root: Optional[str] = None) -> str:
    """Return Mind2Web dataset root, optionally overridden by args."""
    if root:
        return root
...
def format_history(prev_actions: List[str], step_idx: int, max_prev_acts: int, postfix: str) -> str:
    ...
    if 'qwen2' in postfix.lower():
        ...
        _, retained_history = clean_prev_step_instructions[-max_prev_acts:]
        history_str = ' '.join(
            f"Step {i}. {remove_redundant_spaces(instruc.replace('  ', ' ').replace('[', ' ', 1).replace(']', ' ', 1).strip(' .'))}."
            for i, instruc in enumerate(
                retained_history,
                start=max(1, len(clean_prev_step_instructions) - max_prev_acts + 1),
            )
        ...
        history_str = '\n'.join(
            ...
            for i, step in enumerate(
                prev_actions[max(0, step_idx - max_prev_acts):step_idx], start=1
            )
        )
    else:
        history_str = ' '.join(
...
def build_prompt(goal: str, history_str: str, postfix: str, args: argparse.Namespace) -> str:
    """Build prompt based on model type."""
    if 'atlas' in postfix.lower():
        return ATLAS_PROMPT.format(global_task=goal, history='None')
...
def get_model_response(
    ...
    tokenizer_processor: Optional[Tuple] = None,
) -> str:
...
def evaluate_step(
    ...
    try:
        ...
        if (
            action_pred["action_type"] == action_step_ref["action_type"]
            or action_pred["action_type"] == action_step_ref.get("ori_act", "").lower()
        ):
        ...
        if (
            bbox_ref[0] <= click_point[0] / scale <= bbox_ref[2]
            and bbox_ref[1] <= click_point[1] / scale <= bbox_ref[3]
        ):
        ...
        step_result["Op_F1"][0] = squad_metrics.compute_f1(pred_str, ref_str)
        return step_result, True
...
def evaluate_episode(
    ...
    for step_i, step in enumerate(episode["actions"]):
        ...
        prompt = build_prompt(goal, history_str, postfix, args)
        ...
        step_result, success = evaluate_step(
            action_step_ref, bbox_ref, response, postfix, args.scale
        )
        ...
        if success:
            ...
            print(
                f"Op: {step_result['Op_match']} | Elem: {step_result['Ele_match']} | "
                f"Op_F1: {step_result['Op_F1']} | GT:{action_step_ref} <=> "
                f"{parse_predicted_action(response, postfix) if success else 'N/A'}"
            )
...
def print_metrics(task: str, metrics: Dict[str, Any]) -> None:
    """Pretty print the evaluation results."""
    print(f"\nResults for {task}:")
    print(f"Operation F1: {metrics['Operation F1']:.4f}")
    print(f"Element Acc: {metrics['Element Acc']:.4f}")
    print(f"Step Success: {metrics['Step Success']:.4f}")
    print(f"Episode Success: {metrics['Episode Success']:.4f}")
    print(f"Operation F1 by category: {metrics['Operation F1 cate']}")
    print(f"Macro Element Acc: {metrics['Macro Ele Acc']:.4f}")
    print(f"Macro Operation F1: {metrics['Macro Op F1']:.4f}")
    print(f"Macro Step Success Rate: {metrics['Macro Step SR']:.4f}")
    print(f"Average Inference Time per Step: {metrics['time_per_step']:.4f}s")
...
def save_results(
    ...
    save_file = os.path.join(
        eval_result_dir, f"{task}-{time_str}{'_debug' if args.debug else ''}.json"
    )
    output_data = {
        "meta": vars(args),
...
def main() -> None:
    ...
    final_metrics['time_per_step'] = total_inference_time / total_steps if total_steps > 0 else 0

    print_metrics(args.task, final_metrics)

    save_file = save_results(eval_result_dir, args.task, args, final_metrics, results)
    logger.info(
        f"Evaluation finished for {args.pretrained} on Mind2Web {args.task}. "
