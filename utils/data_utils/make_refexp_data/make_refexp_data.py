...
def make_refexp_data():
    ...
    for sample_idx, x in tqdm(enumerate(ds), total=len(ds), desc=DATASET_NAME):
        ...
        if instruc_type_dict[instruc] is None:
            sample = make_elemgnd_sample(task_id=f'autogui_{DATASET_NAME}_elemgnd_{len(samples)}', text=instruc, loc=center_str, output_tag=OUTPUT_TAG)
            elemgnd_cnt += 1
        else:
            if USE_ACTION_PROMPT:
                ...
                sample = make_intentgnd_sample(task_id=f'autogui_{DATASET_NAME}_intentgnd_{len(samples)}', intent=instruc, loc=center_str, output_tag=OUTPUT_TAG)
    ...
    report = f"#samples: {len(samples)}\n#Unique elements: {num_unique_elems}\n#Valid unique images: {num_valid_imgs}\n#All unique images: {len(unique_elems)}\nInvalid elem ratio: {num_invalid_elem} / {len(ds)} = {num_invalid_elem/len(ds):.2f}\elemgnd_cnt: {elemgnd_cnt} | intentgnd_cnt: {intentgnd_cnt} | widgetlist_cnt: 0"
    print(report)

    file_name = os.path.join(SAVE_ROOT, f"{DATASET_NAME}_s{SCALE}_{len(samples)// 1000}k{'_debug' if DEBUG else ''}{'_actformat' if USE_ACTION_PROMPT else ''}.json")
    print(f"save to {file_name}")
    with open(file_name.replace('.json', '_info.json'), "w") as f:
        json.dump({'num_samples': len(samples), '#num_unique_elems': num_unique_elems, '#all_elems': len(ds), '#valid_unique_images': num_valid_imgs, '#all_unique_images': len(unique_elems), 'elemgnd_cnt': elemgnd_cnt, 'intentgnd_cnt': intentgnd_cnt, 'num_invalid_elements': num_invalid_elem, 'invalid_elem_type': {k:len(v) for k,v in invalid_elem.items()}}, f, indent=2)
