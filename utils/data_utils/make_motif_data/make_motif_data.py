...
def make_motif_data():
    data = load_dataset("cjfcsjt/MOTIF_automation_preprocess", split='train_au_tu')
    ...
    for group_idx, x in tqdm(enumerate(data), total=len(data), desc=DATASET_NAME):
        ...
        if len(x['bbox_mc']) == 0: continue

        bbox, instruc = x['bbox_mc'][0], x['instr'].strip()

        elem_text_lengs.append(instruc)
        sample_identifier = f"{x['image_id']}|{x['instr']}"
        ...
        if not SKIP_CHECKING:
            ...
            if abs(bbox[0] - bbox[2]) <= 0.005 or abs(bbox[1] - bbox[3]) <= 0.005:
                ...
                if False:
                    ...
                    cv2.rectangle(img, unnorm_boxes[:2], unnorm_boxes[2:], color=(0,0,255), thickness=2)
        ...
        if USE_ACTION_PROMPT:
            ...
            sample = make_actionplanning_sample(task_id=f'autogui_{DATASET_NAME}_intentgnd_{len(samples)}', global_task=instruc, gt_action=action, history='None', prompt_format_type='aguvis')
        else:
            sample = make_elemgnd_sample(task_id=f'autogui_{DATASET_NAME}_intentgnd_{len(samples)}', text=instruc, loc=center_str, output_tag=OUTPUT_TAG)

        sample['image'], sample['unnormalized_box'], sample['task_attr'] = 'motif/' + x['image_id'], unnorm_boxes, instruc
    ...
    with open(file_name.replace('.json', '_info.json'), "w") as f:
        json.dump({'num_samples': len(samples), '#num_unique_elems': num_unique_elems, '#all_elems': len(data), '#valid_unique_images': num_valid_imgs, '#all_unique_images': len(unique_elems), 'text_loc_cnt': len(samples), 'ocr_cnt': 0, 'elemclass_cnt': 0, 'intentgnd_cnt': 0, 'widgetlist_cnt': 0, 'num_invalid_elements': num_invalid_elem}, f, indent=2)
