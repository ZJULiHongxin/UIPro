...
def make_omniact_data():
    ...
    for sample_idx, sample in tqdm(enumerate(data.values()), total=len(data)):
        ...
        if PLANNING:
            ...
            if 'click' in action_raw:
                ...
                action_str = DRAG_TEMPLATE.format(target_x=max(0, min(SCALE-1, round(x/W*SCALE))), target_y=max(0, min(SCALE-1, round(y/H*SCALE))))
                ...
                action_str = SCROLL_TEMPLATE.format(direction=direction, distance=get_scroll_dist(amount))
            ...
            # add key usage
            sample['conversations'][0]['value'] = sample['conversations'][0]['value'].replace("Now, first describe", "You can use the action {'action_type': 'hotkey', 'key_comb': ['ctrl-shift-n' (open new tab/create a new folder), 'ctrl-a' (select all), 'winleft-a' (open new window), 'win-d' (close current window), 'ctrl-+' (zoom in), 'ctrl--' (zoom out), 'ctrl-f' (find/search), 'ctrl-w' (close tab), 'ctrl-y' (redo), 'alt-f4' (close window), 'ctrl-shift-tab' (previous tab), 'ctrl-tab' (next tab), 'ctrl-t' (new tab), 'ctrl-down' (scroll down), 'ctrl-up' (scroll up), ...]} to press a hotkey.\nYou can use the action {'action_type': 'press_key', 'key': ['space', 'pgdn', 'tab', 'esc', 'up', 'down', 'left', 'right', 'enter', ...]} to press a single key.\n\nNow, first describe")
    ...
    save_to = os.path.join(SAVE_ROOT, f"{DATASET_NAME}-{DEVICE}-{SPLIT}{'_wActRef' if USE_ACTION_REFEXP else ''}_s{SCALE}_{len(samples)}.json")
