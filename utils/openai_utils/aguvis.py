...
type_target_func = {
    ...
    "description": "Input text into a text field. Use this action instead of pyautogui.write.",
    "parameters": {
        "type": "object",
        "properties": {
            "x": {
                "type": "number",
                "description": "The x coordinate of the text field",
            },
            "y": {
                "type": "number",
                "description": "The y coordinate of the text field",
            },
            "text": {
                "type": "string",
...
mind2web_system_message = f"""You are a GUI agent. You are given a task and a screenshot of the screen. You need to perform a series of pyautogui actions to complete the task.

You have access to the following functions:
- {json.dumps(select_option_func)}
- {json.dumps(type_target_func)}
"""
...
class AGUVIS:
    def __init__(self, device, model_name: str = ''):
        ...
        self.processor = Qwen2VLProcessor.from_pretrained(model_name, min_pixels=256*28*28, max_pixels=1008*1008)
    ...
    def test_speed(self, prompt: str, image: str = None, max_new_tokens: int = 512):
        ...
        user_message = {
            "role": "user",
            "content": [
                {'type': 'image_url', 'image': image},
                {'type': 'text', 'text': user_instruction.format(
                    overall_goal=prompt,
        ...
        text = self.processor.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=False, chat_template=chat_template, use_cache=False
        )
        text += recipient_text
        ...
        start = time.time()
        output_ids = self.model.generate(**inputs, temperature=0.0, do_sample=False, max_new_tokens=max_new_tokens)
    ...
    def get_model_response(self, prompt: str, image: str = None, temperature: float = 0.0, max_new_tokens: int = 512, sys_prompt: str = ''):
        ...
        inputs = self.processor(text=[text], images=image_inputs, videos=video_inputs, padding=True, return_tensors="pt")
        inputs = inputs.to(self.model.device)
        cont = self.model.generate(**inputs, temperature=temperature, max_new_tokens=max_new_tokens)
        ...
        prompt = text
        text_outputs = 'Thought: ' + text_outputs
        return text_outputs