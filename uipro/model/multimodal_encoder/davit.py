...
class DaViTVisionTower(nn.Module):
    ...
    def load_model(self, device_map=None):
        ...
        self.image_processor = CLIPImageProcessor.from_pretrained("florence2/preprocessor_config.json", trust_remote_code=True)
    ...
    def forward(self, images):
        # images: (1 + #patches) x 3 x 768 x 768, the values on the images tensor have been normalized with std and mean.
        # use this code to denormalize: images[:,idx,:,:] * self.image_processor.image_std[idx] + self.image_processor.image_mean[idx]
        if type(images) is list:
            ...
            image_features = self.vision_tower.forward_features_unpool(images.to(device=self.device, dtype=self.dtype))

        return image_features # bs x 576 x 1024
    ...
    def hidden_size(self):
        return self.config.embed_dims[-1]
