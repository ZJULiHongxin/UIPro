...
from transformers import AutoProcessor, Pix2StructImageProcessor, Pix2StructProcessor, Pix2StructForConditionalGeneration
...
class Pix2StructLargeVisionTower(nn.Module):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        self.do_resize = False # args.do_resize
        self.de_normalize = True # args.de_normalize # de-normalize the input image and perform preprocessing with pix2struct processor
        self.select_layer = args.mm_vision_select_layer # NOTE: not implemented yet, this parameter has no effect
        self.input_image_size = None # args.input_image_size
    ...
    def load_model(self):
        ...
        self.pix2struct_processor = Pix2StructProcessor.from_pretrained("google/pix2struct-large")
        self.pix2struct_processor.image_processor.is_vqa = False
        #self.pix2struct_processor.size = {'shortest_edge': self.pix2struct_processor.size}

        # https://vscode.dev/github/ZJULiHongxin/highres_autogui/blob/main/llava/mm_utils.py#L176 已经手动在preprocessor外部分好块了，无需经过Pix2StructProcessor里面的分块代码处理。
        ...
        self.pix2struct_processor.image_processor.do_normalize = False
    ...
    def forward(self, x):
        ...
        H, W = x.shape[-2:]
        dtype = x.dtype
        patch_size = self.pix2struct_processor.image_processor.patch_size['height']
        x=self.pix2struct_processor(
            images=x.float(), return_tensors="pt",
            max_patches=H*W//patch_size//patch_size,
            input_data_format=ChannelDimension.FIRST).to(dtype=dtype) # match_patches=2048
        ...
        if self.do_resize:
            ...
            return image_features # bs x 1536 x 32 x 32
        else:
            return image_features # bs x 2025 x 1536
