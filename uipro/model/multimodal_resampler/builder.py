...
from uipro.model.multimodal_resampler.sampler import Resampler, CAbstractor, MplugDocOwlHReducerModel, MultiCompressor
from uipro.model.multimodal_resampler.dabs import DAbstractor
...
class TextGuidedSampler(nn.Module):
    ...
    def __init__(self, projector_type, config):
        ...
        elif projector_type == 'Dabs':
            self.post_qformer = DAbstractor(
                        num_queries=self.grid_size**2,
                        embed_dim=config.mm_hidden_size,
                        output_hidden_size=config.hidden_size,
                    )
