...
def rearrange_image_feature(
    image_feature: torch.Tensor,
    origin_image_width: int,
    origin_image_height: int,
):
    
    # image_feature: torch.Size([batch_size, sequence_length, hidden_size])
    batch_size, sequence_length, hidden_size = image_feature.size() # batch_size, 576, 1024
    resized_height, resized_width, resized_patch_height, resized_patch_width = \
    adapt_size(origin_image_height,origin_image_width)
    num_patches = resized_patch_height*resized_patch_width
    
    image_feature = image_feature[:, :num_patches, :]
    
    image_feature = image_feature.reshape(batch_size, resized_patch_height, resized_patch_width, hidden_size)
    
    return image_feature

def debug_rerange_image_feature():
    origin_image_height, origin_image_width = 1000, 2000
    resized_height, resized_width, resized_patch_height, resized_patch_width = \
    adapt_size(origin_image_height,origin_image_width)
    image = torch.zeros(1, 3, resized_height, resized_width)
    image = torch.nn.functional.unfold(image, (14, 14), stride=(14, 14))
    for i in range(resized_patch_height*resized_patch_width):
        image[:, :, i] = i+1
    image = torch.nn.functional.fold(image, (224, 462), (14,14), stride=14)
    image = image.squeeze(0)
    print(image.shape) # torch.Size([3, 224, 462])
    print(image[0])
    num_patches_to_pad = MAX_PATCHES - resized_patch_height*resized_patch_width
    image = torch_extract_patches(image,PATCH_SIZE, PATCH_SIZE)
    image = image.reshape([resized_patch_width*resized_patch_height,TOKEN_LENGTH])
    ...
    # clip-vit
    encoder = torch.nn.AdaptiveAvgPool2d((24, 24))
    image_feature = encoder(image)
    image_feature = image_feature.flatten(2).transpose(1, 2)
    
    image_feature = rerange_image_feature(image_feature, origin_image_width, origin_image_height)
    
    print(image_feature.shape) # torch.Size([1, 16, 33, 3])
    print(image_feature[0, :, :, 0])
    

if __name__ == "__main__":
    debug_rerange_image_feature()
    pass