...
from uipro.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from torch.utils.data import Dataset
from uipro.train.llava_trainer import LLaVATrainer

from uipro import conversation as conversation_lib
from uipro.model import *
from uipro.mm_utils import tokenizer_image_token, process_anyres_image
from uipro.process_image import process_image_naive, process_image_any_res
...
class ModelArguments:
    ...
    mm_learnable_compressor: Optional[int] = field(default=-1)
    compressor_aggr_type: Optional[str] = field(default='avg')
...
def preprocess_gemma(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            role = roles[sentence["from"]]
            assert role == conv.roles[j % 2], source
            conv.append_message(role, sentence["value"])
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    im_start, im_end, img_token = tokenizer.additional_special_tokens_ids # ['<|im_start|>', '<|im_end|>', '<image>']
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        elif data_path == 'pretrain':
            dataset_paths = [
                'data/blip_laion_cc_sbu_558k.json',
            ]
        elif data_path == 'finetune':
            dataset_paths = [
                'data/sharegpt4v_mix665k_cap23k_coco-ap9k_lcs3k_sam9k_div2k_sample.json',
                'data/SMR_sample.json',
            ]
        else:
        ...
        rank0_print(Fore.YELLOW + f'A total of {len(self.list_data_dict)} training samples from these datasets: ' + ', '.join(dataset_paths) + Style.RESET_ALL)
...
def train(attn_implementation="flash_attention_2"):
    ...
    if model_args.vision_tower is not None:
        base_model_name = config._name_or_path.lower()
        ...
        rank0_print(Fore.YELLOW + f"Base model name: {base_model_name}. Using model class {str(type(model))}" + Style.RESET_ALL)
    else:
        ...
        if model_args.tune_mm_mlp_adapter:
            ...
            if not model_args.use_global_only:
                for p in model.get_model().sampler.parameters():
                    p.requires_grad = True
            if model_args.mm_projector_type not in ['linear'] and model_args.mm_learnable_gated > -1:
                for i, module in enumerate(model.get_model().mm_projector.expert_ffn):
                    if i != model_args.mm_learnable_gated:
                        for p in module.parameters():
                            p.requires_grad = False

        if model_args.mm_learnable_compressor > -1:
            for i, compressor in enumerate(model.get_model().sampler.post_qformer.projector_list):
                if i != model_args.mm_learnable_compressor:
    ...
    if local_rank == 0:
        ...
        exp_config['model_args']['model_class'] = str(type(model))
    ...
    # Save the vision tower processor config if the vit type is not clip vit
    try:
        model.get_vision_tower().image_processor.save_pretrained(training_args.output_dir)
        print("Vision tower processor config saved successfully.")
